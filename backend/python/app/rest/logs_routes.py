...
def filter_logs():
    ...
    if email and (start_date and end_date):
        ...
        if type(email) is not str or (
            type(start_date) is not datetime or type(end_date) is not datetime
        ):
            return (
                jsonify(
                    {
                        "error": "start_date and end_date query parameter must be datetime and email must be a string"
                    }
                ),
                400,
            )
        ...
        if type(start_date) is not datetime or type(end_date) is not datetime:
            return (
                jsonify(
                    {
                        "error": "start_date and end_date query parameter must be datetime"
                    }
                ),
                400,
            )
